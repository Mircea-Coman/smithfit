"""
    ...
    ===========
      ...
      -----------
        ...
        Python implementation of the NLQFIT8 algorithm. The input parameter
        trmode specifies the resonance type (normally "transmission").

        This version fits real and imaginary parameters that describe
        a frequency dependence of the background (leakage vector). This
        is typically used when a transmission resonance sits on the
        'tail' of another resonance.
...
def optimisefit8(F, S, N, Fseed, sv, loop_plan, Tol, quiet):
    """
       Iterative non-linear fit, NLQFIT8 Step (2)
        -----------------------------------------
         ...
         Uses the results of the initial fit (sv) as the starting values for the iteration.
         ...
         Input parameters:
            ...
            loop_plan - String of characters which defines order of
                        steps used by the fitting process, e.g. 'fwfwc':
                         ...
                         c - repeated fit, iterating until convergence is
                             obtained.
                         w - re-calculate weighting factors on basis of
                             previous fit.
         ...
         Output (MAT58 eqn. 43)
               (list of fitted parameters): [m1, m2, m4, m4, m8, m9, QL, FL]
    """
    ...
    for op in loop_plan:
        ...
        while not (TerminationConditionMet):
            ...
            for i in range(N):
                ...
                t = 2*(F[i]-FL)/FL;
                ip = PV[i]
                E = S[i] - complex(m1,m2) - complex(m8,m9)*t - complex(m3,m4)/den
        ...
        if not quiet: print()
    return [m1,m2,m3,m4,m8,m9,m5,m5*Flwst/m6], weighting_ratio, number_iterations, RMS_Error
...
def GetUnloadedData(mv, scaling_factor_A, trmode, quiet):
    ...
    if trmode=='transmission':
        ...
        Q0 = m5/(1.0-cal_diam)
        return 0, (Q0, cal_diam, cal_gamma_V, cal_gamma_T)
